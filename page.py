...
class TestPage(Assertions):
    ...
    def join_call(self):
        ...
        self.setup()
        self.pause(8)
        self.highlightByXpath(self.driver.find_element_by_xpath(td.Log_in_button_xpath))
        self.clickElementByXPATH(td.Log_in_button_xpath)
        self.typeTextByXPath(td.user_name, td.Email_box_xpath)
        self.typeTextByXPath(td.password, td.Pwd_box_xpath)
        self.clickElementByXPATH(td.log_in_xpath)
    ...
    def calender_sync(self):
        try:
            ...
            self.pause(2)
            audLEVEL0 = self.get_element_text_byindex_xpath(td.tab_audio_level, 0)
            self.highlightByXpath(td.tab_audio_level)
            self.pause(1)
            self.close_window()
            self.switch_to_window(0)
            self.pause(2)
    ...
    def data_from_webRtc(self):
        ...
        self.highlightByXpath(td.tab_videobytes_persec_after_off)
        self.pause(2)
        self.clickElementByXPATH(td.audiosourcehead)
        self.highlightByXpath(td.audiosourcehead)
        self.pause(2)
    ...
    def packet_loss(self):
        ...
        res_v1 = self.calculate_screensize(resolution)
        commands = str(os.path.abspath('clumsy-0.2-win64/clumsy.exe') + " " + self.read_data_fromfile("TestDataSection",
                                                                                                      "clumzy_cmd_run"))
        shell.ShellExecuteEx(lpVerb='runas', lpFile='cmd.exe', lpParameters='/c ' + commands)
        self.pause(20)
    ...
    def calender_sync1(self):
        try:
            ...
            self.clickElementByXPATH(td.log_in_xpath)
            self.pause(10)
            self.highlightByXpath(td.sync_calender_xpath)
            self.clickElementByXPATH(td.sync_calender_xpath)
            self.pause(4)
            self.switch_to_window(1)
            self.clickElementByXPATH(td.google_xpath)
